...
test("known set of unitless-valued CSS properties matches MDN data", () => {
	expect(new Set(propertiesAcceptingUnitless)).toEqual(unitlessCSSProperties);
});

test.each(propertiesAcceptingUnitless)(
	"regexp matches unitless-valued '%s' CSS property",
	(property) => {
		expect(acceptsUnitless.test(property)).toBe(true);
	},
);
...
test.each(nonUnitlessCSSProperties)(
	...
	(property) => {
		expect(acceptsUnitless.test(property)).toBe(false);
	},
);
